...
class StagingRemoveCommand(osc.commandline_git.GitObsCommand):
    ...
    def _remove_submodules(self, git: 'gitea_api.Git', fwd_prs_to_remove: list, context: dict, force: Optional[bool] = False, fork: Optional[str] = None):
        ...
        if fork:
            try:
                fork_owner, fork_repo_name = fork.split('/')
                ...
                print(f"{tty.colorize('ERROR', 'red,bold')}: Invalid or inaccessible fork '{fork}': {e}", file=sys.stderr)
                sys.exit(1)
        ...
        for fwd_pr_to_remove in fwd_prs_to_remove:
            ...
            pkg_prs =  context["fwd_to_package"].get(fwd_pr_to_remove)
            for pkg_pr_org, pkg_pr_repo, pkg_pr_num in pkg_prs:
                removed_refs_str += f"- {org}/{repo}!{num} ({pkg_pr_org}/{pkg_pr_repo}!{pkg_pr_num})\n"

        commit_message = f"Unstaged PRs\n\n{removed_refs_str}"
        git.commit(commit_message)
        git.push(remote, branch=grouped_pr_obj.head_branch, force=force)
    ...
    def run(self, args):
        ...
        self._remove_submodules(git, fwd_prs_to_remove, context, fork=args.fork, force=args.force)
